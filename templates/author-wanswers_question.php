 ...
 *
 * Served at /questions/author/{username}/ via the wanswers_author_name query var.
...
$author_nicename = sanitize_user( get_query_var( 'wanswers_author_name', '' ) );
...
wp_localize_script( 'wanswers-script', 'WANSWERS', wanswers_js_config() );

if ( ! $profile_user ) {
    status_header( 404 );
    get_header();
    echo wp_kses_post( '<main class="cc-qa-single-page"><div class="page-wrap cc-qa-wrap">
...
$browse_url = get_post_type_archive_link( 'wanswers_question' );
...
// ── Stats ──────────────────────────────────────────────────────────────────
$q_count = (int) count_user_posts( $uid, 'wanswers_question' );
...
foreach ( (array) $answers_query->posts as $aid ) {
    if ( get_post_meta( $aid, '_wanswers_accepted', true ) ) $accepted_count++;
}
...
Wanswers_Database::backfill_lifetime_votes( $uid );

$lifetime_up   = (int) get_user_meta( $uid, '_wanswers_lifetime_upvotes',   true );
$lifetime_down = (int) get_user_meta( $uid, '_wanswers_lifetime_downvotes', true );
...
$lb_html       = $show_lb ? Wanswers_Leaderboard::render_inline( 0, $lb_is_sidebar ? 'compact' : 'stacked' ) : '';

// ── Badges + Chart ─────────────────────────────────────────────────────────
$badge_html  = Wanswers_Badges::render_pills( $uid );
$chart_html  = Wanswers_Badges::render_activity_chart( $uid );
$profile_url = Wanswers_Badges::profile_url( $uid );
...
add_action( 'wp_head', function() use ( $name, $q_count, $a_count, $accepted_count, $profile_url ) {
    $parts = array();
    if ( $q_count )        $parts[] = $q_count . ' ' . _n( 'question', 'questions', $q_count, 'wanswers-seo-first-qa' );
    if ( $a_count )        $parts[] = $a_count . ' ' . _n( 'answer', 'answers', $a_count, 'wanswers-seo-first-qa' );
  ...
  <div class="page-wrap cc-qa-wrap cc-qa-profile" id="cc-qa-app">
    ...
    <section class="qa-profile-section">
      ...
      <div class="qa-chart-wrap">
        <?php echo wp_kses_post( $chart_html ); ?>
      </div>
    </section>
      ...
      <?php if ( ! empty( $questions ) ) : ?>
        <div class="qa-profile-questions">
          <?php foreach ( $questions as $q ) :
            $q_votes  = (int) get_post_meta( $q->ID, '_wanswers_votes', true );
            $q_ans    = (int) get_post_meta( $q->ID, '_wanswers_answer_count', true );
            $q_acc    = (bool) get_post_meta( $q->ID, '_wanswers_accepted', true );
            $q_topics = wp_get_object_terms( $q->ID, 'wanswers_question_topic' );
          ?>
      ...
      <?php if ( ! empty( $answers ) ) : ?>
        <div class="qa-profile-answers">
          <?php foreach ( $answers as $ans ) :
            $a_votes    = (int) get_post_meta( $ans->ID, '_wanswers_votes', true );
            $a_accepted = (bool) get_post_meta( $ans->ID, '_wanswers_accepted', true );
