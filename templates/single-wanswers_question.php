...
$author      = Wanswers_Shortcode::get_author_display( $question->post_author );
...
$accepted_id = (int) get_post_meta( $question_id, '_wanswers_accepted_answer', true );
$user_voted  = $user_id ? Wanswers_Database::user_voted( $question_id, $user_id ) : false;
...
$answers_per_page  = (int) Wanswers_Admin::get( 'wanswers_answers_per_page' );
...
usort( $answers, function( $a, $b ) {
    $a_acc = (int) get_post_meta( $a->ID, '_wanswers_accepted', true );
    $b_acc = (int) get_post_meta( $b->ID, '_wanswers_accepted', true );
    return $b_acc - $a_acc;
} );
...
foreach ( $answers as $a ) {
    $a_author = Wanswers_Shortcode::get_author_display( $a->post_author );
    $entry    = array(
        '@type'       => 'Answer',
        'text'        => wp_strip_all_tags( $a->post_content ),
        'dateCreated' => get_the_date( 'c', $a ),
        'upvoteCount' => (int) get_post_meta( $a->ID, '_wanswers_votes', true ),
...
add_action( 'wp_head', function() use ( $question, $a_count, $votes ) {
    ...
    $desc .= ' · ' . $a_count . ' ' . _n( 'answer', 'answers', $a_count, 'wanswers-seo-first-qa' ) . ' · ' . $votes . ' votes';
      ...
      <div class="qa-detail-layout">
        ...
        <div class="qa-vote-col">
          <button class="qa-vote-btn qa-vote-up <?php echo esc_attr( $user_voted ? 'voted' : '' ); ?>"
                  ...
                  <?php echo esc_attr( ( ! is_user_logged_in() || $is_author ) ? 'disabled' : '' ); ?>>▲</button>
        ...
        <div class="qa-detail-content">
          ...
          <?php
          $q_perms = Wanswers_Shortcode::get_edit_permissions( $question );
          if ( $q_perms['can_edit'] ) : ?>
          ...
          <div class="qa-detail-meta">
            ...
            <?php if ( (int) get_post_meta( $question_id, '_wanswers_accepted', true ) ) : ?>
    ...
    <section class="qa-answers-section" aria-label="Answers">
      ...
      <div class="qa-answers-header">
        <h2 class="qa-answers-title">
          ...
          <?php echo esc_html( $a_count === 1 ? __( 'Answer', 'wanswers-seo-first-qa' ) : __( 'Answers', 'wanswers-seo-first-qa' ) ); ?>
        </h2>
        <?php if ( $a_count > 1 ) : ?>
      ...
      <div class="qa-answers-list" id="answers-list-<?php echo esc_attr( $question_id ); ?>">
        ...
        if ( ! empty( $answers ) ) {
            foreach ( $answers as $answer ) {
                Wanswers_Shortcode::render_answer_card( $answer, $user_id, $is_author, $accepted_id );
            }
        } else {
            echo wp_kses_post( '<div class="qa-no-answers"><span>No answers yet.</span> <strong>Be the first!</strong></div>' );
        }
        ?>
      </div>
